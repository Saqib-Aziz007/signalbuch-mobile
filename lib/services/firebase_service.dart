...
class FirebaseService {
  ...
  Future<List<Article>> getRelatedArticlesByCategory(
      Article article, int limit) async {
    List<Article> data = [];
    await firestore
        .collection('articles')
  ...
  Future<List<Article>> getRelatedArticlesByAudio(
      Article article, int limit) async {
    List<Article> data = [];
    await firestore
        .collection('articles')
  ...
  Future<List<Article>> getArticlesByAuthorId(
  ...
  Future<QuerySnapshot> getArticlesSnapshotByPopular(
  ...
  Future<QuerySnapshot> getArticlesSnapshotByLatest(
      {DocumentSnapshot? lastDocument}) async {
    QuerySnapshot snapshot;
    if (lastDocument == null) {
      snapshot = await firestore
  ...
  Future<QuerySnapshot> getVideoArticlesSnapshot(
      {DocumentSnapshot? lastDocument, required ArticlesBy articlesBy}) async {
    final String orderBy =
        articlesBy == ArticlesBy.popular ? 'views' : 'created_at';
    QuerySnapshot snapshot;
    if (lastDocument == null) {
      snapshot = await firestore
          .collection('articles')
  ...
  Future<QuerySnapshot> getCommentsSnapshot(
      {required String articleId, DocumentSnapshot? lastDocument}) async {
    QuerySnapshot snapshot;
    if (lastDocument == null) {
      snapshot = await firestore
          .collection('comments')
  ...
  Future<int> getAuthorCommentsCount(String auhtorId) async {
    ...
    final AggregateQuerySnapshot snap = await collectionReference
        .where('article_author_id', isEqualTo: auhtorId)
        .count()
        .get();
